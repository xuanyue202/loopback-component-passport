...
PassportConfigurator.prototype.buildUserLdapProfile = function(user, options) {
  var profile = {};
  for (var profileAttrName in options.profileAttributesFromLDAP) {
    var ldapAttrName = options.profileAttributesFromLDAP[profileAttrName];

    // Active Directory UUIDv4 compatibility
    if (ldapAttrName === 'objectGUID' || ldapAttrName === 'objectSid') {
      if (user[ldapAttrName].length == 14) {
        profile[profileAttrName] = bytesToUuid(
          Array.prototype.slice.call(Buffer.from(user[ldapAttrName])));
      }
    }

    // [contact schema](https://tools.ietf.org/html/draft-smarr-vcarddav-portable-contacts-00) compatibility
    if (ldapAttrName === 'givenName') {
      if (!profile['name']) profile['name'] = {};
      profile['name'].givenName = user[ldapAttrName];
    }
    if (ldapAttrName === 'sn') {
      if (!profile['name']) profile['name'] = {};
      profile['name'].familyName = user[ldapAttrName];
    }
    if (ldapAttrName === 'mail') {
      profile.emails = [{value: user[ldapAttrName]}];
    }
  ...
  if (!profile.id) {
    profile.id = user['uid'] || bytesToUuid(
      Array.prototype.slice.call(Buffer.from(user['objectGUID'])));
  }
  if (!profile.emails) {
...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var callbackPath = options.callbackPath || ((link ? '/link/' : '/auth/') +
    name + '/callback');
  var callbackHTTPMethod = options.callbackHTTPMethod;
          ...
          req.logIn(user, function(err) {
            ...
            if (info && info.accessToken) {
              if (!!options.json) {
                ...
                // maxAge is in ms
                var maxAge = 1000 * info.accessToken.ttl;
                if (info.accessToken.ttl === -1) {
